...
**不要在 adapter 里自己再维护一份归一化映射表**——曾经在 `ats` 里维护过一份，
漏了 Lever 那种带空格的 `"Full Time"`，映射不中直接落成 NULL，实测丢了 8 条。
...
测试**不发任何网络请求**：`tests/conftest.py` 里的 `FakeFetcher` 按 URL 回放
`tests/fixtures/` 下的样本，所以跑得快、结果稳定，源站改版也不会让测试
变成随机失败。

fixture 里的值全是合成的（公司名、邮箱、钱包地址一律编造），
但**结构严格照抄真实响应**——字段名、嵌套层级，以及各站那些反直觉的坑都原样
保留。本仓库是公开的，真实响应里的招聘方联系方式不进 git，所以宁可自己编数据。

用例优先覆盖那些**坏了也不会报错**的地方：水位线该不该提前退出、
接口静默降级时有没有被识破、一页多个 JobPosting 时挑没挑对、
容错路径会不会把整轮采集带崩。给源改行为时，先看这里有没有对应的用例。
